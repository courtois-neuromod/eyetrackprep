...
"""
Assign run number to runs labelled by subtask for retino and fLoc task
...
"""
List of valid CNeuroMod subjects (to filter out test files w placeholder sub numbers)
"""
...
def compile_rawfile_list(
    ...
    """
    Exports a DataFrame with a compiled overview of 
    all available eye-tracking files for a given dataset
    (pupils.pldata, gaze.pldata and eye0.mp4 exported 
    by pupil, psychopy log file) to support manual QCing.
    
    Returns a list of paths to all the directories 
    with existing pupil.pldata
    """
    ...
    # Friends has no events files, compiled differently 
    # TODO: expand script to newer tasks like narratives, multfs... (check structure)
    if task_root == 'friends':
        df_files, pupil_file_paths = process_friends(
            df_files, ses_list)
    else:
        df_files, pupil_file_paths = process_dset(
            df_files,
            task_root,
            ses_list,
        )
...
def main() -> None:
    ...
    '''
    file_report, pupil_paths = compile_rawfile_list(
        in_path, out_path)
